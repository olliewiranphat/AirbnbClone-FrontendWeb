...
function BookingHistory() {
   ...
   const fetchBookings = async () => {
      try {
         setLoading(true);
         // Assuming you have the customer ID available - you might need to get this from context, localStorage, etc.
         // const customerID = localStorage.getItem('customerID'); // Adjust based on how you store user info

         const customerID = userData?.clerkID;
         ...
         const response = await axios.get(`http://localhost:8081/user/bookings/customer/1`);
         console.log('response', response)
         if (response.data.success) {
            setBookings(response.data.data);
         } else {
            setError('Failed to fetch bookings: ' + response.data.message);
         }
      } catch (err) {
         setError('Error fetching bookings: ' + err.message);
      } finally {
