...
function AccomStep2() {
    const { formData, setFormDataByFIeld } = useAccomStore();
        const handleChange = (e) => {
            const { name, value } = e.target;
            setFormDataByFIeld(value, name);
        };
  return (
    <div className='flex flex-col gap-12 mt-4 w-[60%] h-[100%] bg-pink-50 border-none p-8 rounded-2xl shadow-xl'>
      ...
      <BasicInfoForm
              ...
              setSelectedType={(type) => setFormDataByFIeld({ availQTY: type[0], numBedrooms: type[1], numBathrooms: type[2], maxGuests: type[3] }, 'availQTY')}
            />
      </div>
        {/* Amenities Selector */}
      ...
      <div>
        ...
        <div className='flex justify-between gap-4 mt-2 p-6 w-full space-y-6'>
            ...
            <input type="text" name='price' defaultValue={formData.pricePerNight} onChange={handleChange} placeholder='What is the price per night?' className="input input-bordered border-[#a4a5a5] w-3/4 rounded-xl textarea-sm mb-2" />
        </div>
    </div>
  )
}
